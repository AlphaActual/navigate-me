...
export default {
  ...
  methods: {
    async getMyLocation(){
      let coordinates;
      try{
        coordinates = await this.$getLocation({enableHighAccuracy: true, timeout: 5000, maximumAge: 0});
      }
      catch(err){
        console.log(err);
      }
    ...
    startTracking(){
      ...
      let nearestWPIndex = this.waypoints.findIndex(wp=>wp.id === nearestWP.id);
      let courseToNearestWP = this.calculateCourse({lat:this.boatPosition[0],lng:this.boatPosition[1]},nearestWP);
      ...
      if(this.compareHeadings(courseToNearestWP,nearestWP.nextCourse)){
        ...
        // }
        while (nearestWPIndex < this.waypoints.length -1 && !this.compareHeadings(courseToNearestWP,nearestWP.nextCourse)){
          nearestWPIndex++;
          nearestWP = this.waypoints[nearestWPIndex];
          courseToNearestWP = this.calculateCourse({lat:this.boatPosition[0],lng:this.boatPosition[1]},nearestWP);
          console.log("next course:",nearestWP.nextCourse);
          console.log(courseToNearestWP);
          console.log("difference:",Math.abs(nearestWP.nextCourse - courseToNearestWP));
          console.log(this.compareHeadings(courseToNearestWP,nearestWP.nextCourse));
        };
        this.boatPolyline = [this.boatPosition,[nearestWP.lat,nearestWP.lng]];
      }
